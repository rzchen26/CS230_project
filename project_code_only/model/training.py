...
def train_sess(sess, model_spec, num_steps, writer, params):
    ...
    for i in t:
        ...
        t.set_postfix(loss='{:10.6f}'.format(loss_val))


    metrics_values = {k: v[0] for k, v in metrics.items()}
    metrics_val = sess.run(metrics_values)
    metrics_string = " ; ".join("{}: {:10.6f}".format(k, v) for k, v in metrics_val.items())
...
def train_and_evaluate(train_model_spec, eval_model_spec, model_dir, params, restore_from=None):
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(begin_at_epoch, begin_at_epoch + params.num_epochs):
            ...
            eval_acc = metrics['similarity_progress']
            if eval_acc >= best_eval_acc:
                # Store new best similarity_progress
                best_eval_acc = eval_acc
                # Save weights
                best_save_path = os.path.join(model_dir, 'best_weights', 'after-epoch')
                best_save_path = best_saver.save(sess, best_save_path, global_step=epoch + 1)
                logging.info("- Found new best similarity_progress, saving in {}".format(best_save_path))
